...
static void *cas_lazy_thread_fn(void *data)
{
...
static inline void _cache_mngt_async_context_init(
		struct _cache_mngt_async_context *context)
{
	env_completion_init(&context->cmpl);
...
static int _cache_mngt_lock_sync(ocf_cache_t cache)
{
	...
	context = env_malloc(sizeof(*context), 0);
	if (!context)
		return -ENOMEM;
	...
	result = env_completion_wait_interruptible(&context->cmpl);
...
/*
 * Since env_completion_wait() is used, hang tasks may occure if flush would
 * take long time.
 */
static int _cache_mngt_cache_flush_uninterruptible(ocf_cache_t cache)
{
	...
	env_atomic_set(&cache_priv->flush_interrupt_enabled, 0);

	ocf_mngt_cache_flush(cache, _cache_mngt_cache_flush_uninterruptible_complete,
			&context);
	env_completion_wait(&context.cmpl);

	env_atomic_set(&cache_priv->flush_interrupt_enabled, 1);
		...
		void (*compl)(ocf_cache_t cache))
{
	...
	_cache_mngt_async_context_init(context);
	context->compl_func = compl;
	env_atomic_set(&cache_priv->flush_interrupt_enabled, interruption);
...
static void _cache_mngt_cache_priv_deinit(ocf_cache_t cache)
{
	...
	env_free(cache_priv->stop_context);
...
static int _cache_mngt_cache_stop_sync(ocf_cache_t cache)
{
	...
	result = env_completion_wait_interruptible(&context->async.cmpl);
...
static void mark_core_id_used(ocf_cache_t cache, uint16_t core_id)
{
	...
	env_bit_set(core_id, (unsigned long *)bitmap);
}
...
static void mark_core_id_free(ocf_cache_t cache, uint16_t core_id)
{
	...
	env_bit_clear(core_id, (unsigned long *)bitmap);
}
...
int cache_mngt_core_pool_remove(struct kcas_core_pool_remove *cmd_info)
{
	...
	int ret = cas_blk_identify_type(cmd_info->core_path_name, &volume_type_id);
	if (ret)
		return ret;
		...
		struct kcas_insert_core *cmd_info)
{
	...
	env_completion_init(&add_context.cmpl);
		...
		struct kcas_io_classes *cfg)
{
	...
	io_class_cfg = env_zalloc(sizeof(struct ocf_mngt_io_class_config) *
			OCF_USER_IO_CLASS_MAX, 0);
	if (!io_class_cfg)
...
int cas_init_mngt_queue_thread(ocf_queue_t q)
{
...
int cas_init_io_queue_thread(ocf_queue_t q)
{
	ocf_cache_t cache = ocf_queue_get_cache(q);
	const char *cache_num = ocf_cache_get_name(cache) + 5;
	char name[48] = {};
...
static int _cache_mngt_start_queues(ocf_cache_t cache)
{
	...
	for (i = 0; i < cpus_no; i++) {
		...
		result = cas_init_io_queue_thread(cache_priv->io_queues[i]);
		if (result) {
	...
	result = cas_init_mngt_queue_thread(cache_priv->mngt_queue);
	if (result) {
		...
		ocf_cache_t cache)
{
	...
	struct vb_object *bd_cache_obj;

	cache_obj = ocf_cache_get_volume(cache);
	ENV_BUG_ON(!cache_obj);

	bd_cache_obj = cas_volume_get_vb_object(cache_obj);
...
static void _cache_mngt_start_complete(ocf_cache_t cache, void *priv, int error)
{
	...
	if (error == -OCF_ERR_NO_FREE_RAM) {
		result = cache_mngt_create_cache_device_cfg(&ctx->device_cfg,
				ctx->cache_path);
		if (result) {
			syslog(LOG_WARNING, "Cannot calculate amount of DRAM "
					"needed\n");
			ctx->min_free_ram = 0;
		} else {
...
static int _cache_mngt_cache_priv_init(ocf_cache_t cache)
{
	...
	cache_priv->stop_context = env_malloc(sizeof(*cache_priv->stop_context), 0);
		...
		struct ocf_metadata_probe_status *status)
{
	...
	*context->result = error;
	...
	strncpy(context->cache_name_meta, status->cache_name, OCF_CACHE_NAME_SIZE - 1);
	context->cache_name_meta[OCF_CACHE_NAME_SIZE - 1] = '\0';
		...
		struct kcas_standby_activate *cmd)
{
	...
	context = env_zalloc(sizeof(*context), 0);
	if (!context) {
		result = -ENOMEM;
		goto out_cache_unlock;
	}
...
finalize_err:
	...
	ocf_mngt_cache_stop(cache, _cache_mngt_cache_stop_rollback_complete,
			context);
	rollback_result = env_completion_wait_interruptible(&context->async.cmpl);
		...
		struct kcas_start_cache *cmd)
{
	...
	case CACHE_INIT_STANDBY_LOAD:
		...
		result = 0;
		strncpy(cfg->name, cache_name_meta, OCF_CACHE_NAME_SIZE - 1);
		cfg->name[OCF_CACHE_NAME_SIZE - 1] = '\0';
...
int cache_mngt_interrupt_flushing(const char *cache_name, size_t name_len)
{
	...
	if (env_atomic_read(&cache_priv->flush_interrupt_enabled))
		ocf_mngt_cache_flush_interrupt(cache);
...
int cache_mngt_get_core_info(struct kcas_core_info *info)
{
	...
	if (uuid->data) {
		strncpy(info->core_path_name, uuid->data,
...
static int cache_mngt_wait_for_rq_finish_visitor(ocf_core_t core, void *cntx)
{
	...
	struct vb_object *bdobj = cas_volume_get_vb_object(obj);

	while (env_atomic64_read(&bdobj->pending_rqs))
		usleep(5000);
