...
#define _GET_CMD_INFO(cmd_info, arg, size) ({ \
	cmd_info = env_malloc(size, 0); \
	if (!cmd_info) \
		return -ENOMEM; \
	memcpy(cmd_info, arg, size); \
})

#define GET_CMD_INFO(cmd_info, arg) _GET_CMD_INFO(cmd_info, arg, sizeof(*cmd_info))

#define RETURN_CMD_RESULT(cmd_info, arg, result) ({ \
	int ret = result; \
	cmd_info->ext_err_code = abs(result); \
	memcpy(arg, cmd_info, sizeof(*cmd_info)); \
	env_free(cmd_info); \
...
/*********************************************/
long cas_service_ioctl_ctrl(unsigned int cmd, void *arg)
{
	int retval = 0;
	...
	case KCAS_IOCTL_PARTITION_SET: {
		...
		_GET_CMD_INFO(cmd_info, arg, KCAS_IO_CLASSES_SIZE);//TODO:change
