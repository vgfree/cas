...
int vcaslog(int log_level, const char *template, va_list args)
{
	...
	fprintf(log, "%s cascli: ", timestamp);
...
int get_core_info(uint32_t cache_id, int core_id,
		  struct kcas_core_info *info, bool by_id_path)
{
	...
	if (run_ioctl(KCAS_IOCTL_CORE_INFO, info, sizeof(*info)) < 0) {
		return FAILURE;
	}
...
static int get_core_device(uint32_t cache_id, int core_id, struct core_device *core, bool by_id_path)
{
	...
	if (get_core_info(cache_id, core_id, &cmd_info, by_id_path)) {
...
int get_cache_count(void)
{
	...
	if (run_ioctl(KCAS_IOCTL_GET_CACHE_COUNT, &cmd, sizeof(cmd)) < 0)
		return 0;
...
uint32_t *get_cache_ids(int *caches_count)
{
	...
	*caches_count = 0;
	...
	do {
		if ((status = run_ioctl(KCAS_IOCTL_LIST_CACHE, &cache_list, sizeof(cache_list))) < 0) {
			if (errno != EINVAL) {
 ...
 * @return valid pointer to a structure or NULL if error happened
 */
struct cache_device *get_cache_device_by_id_fd(uint32_t cache_id, bool by_id_path)
{
	...
	if (run_ioctl(KCAS_IOCTL_CACHE_INFO, &cmd_info, sizeof(cmd_info)) < 0) {
		if (errno != EINVAL)
			return NULL;
	}
...
struct cache_device **get_cache_devices(int *caches_count, bool by_id_path)
{
	...
	*caches_count = count = get_cache_count();
	if (count <= 0) {
		goto error_out;
	}
...
int attach_cache(uint32_t cache_id, const char *cache_device, int force)
{
...
int dump_inflight_of_cache(uint32_t cache_id)
{
	...
	if (run_ioctl(KCAS_IOCTL_DUMP_INFLIGHT, &cmd, sizeof(cmd)) < 0) {
 ...
 */
int get_cache_mode(uint32_t cache_id, int *mode)
{
	struct kcas_cache_info cmd_info;
...
int set_cache_mode(unsigned int cache_mode, uint32_t cache_id, int flush)
{
	...
	if (get_cache_mode(cache_id, &orig_mode)) {
	...
	if (flush_param_required) {
		if (1 == flush) {
			...
			cas_printf(LOG_INFO, "CAS is currently migrating from %s to %s mode.\n"
				...
				"Please find flushing progress via statistics command (‘cascli -P’).\n",
	...
	if (run_ioctl_interruptible_retry(KCAS_IOCTL_SET_CACHE_STATE, &cmd, sizeof(cmd), "Setting mode",
		...
		struct cas_param *params)
{
	...
	if (get_cache_mode(cache_id, &cache_mode)) {
		return FAILURE;
	}
	...
	for (i = 0; params[i].name; ++i) {
		...
		if (run_ioctl(KCAS_IOCTL_SET_CORE_PARAM, &cmd, sizeof(cmd)) < 0) {
		...
		struct cas_param *params, unsigned int output_format)
{
	...
	for (i = 0; params[i].name; ++i) {
		...
		if (run_ioctl(KCAS_IOCTL_GET_CORE_PARAM, &cmd, sizeof(cmd)) < 0) {
...
int cache_params_set(uint32_t cache_id, struct cas_param *params)
{
	...
	for (i = 0; params[i].name; ++i) {
		...
		if (run_ioctl(KCAS_IOCTL_SET_CACHE_PARAM, &cmd, sizeof(cmd)) < 0) {
			if (cmd.ext_err_code == OCF_ERR_CACHE_STANDBY)
				print_err(cmd.ext_err_code);
			return FAILURE;
		}
		...
		unsigned int output_format)
{
	...
	for (i = 0; params[i].name; ++i) {
		...
		if (run_ioctl(KCAS_IOCTL_GET_CACHE_PARAM, &cmd, sizeof(cmd)) < 0) {
			if (cmd.ext_err_code == OCF_ERR_CACHE_NOT_EXIST)
				cas_printf(LOG_ERR, "Cache id %"PRIu32" not running\n", cache_id);
 ...
 */
int illegal_recursive_core(uint32_t cache_id, const char *core_device, int core_path_size)
{
	...
	while (true) {
		...
		cache = get_cache_device_by_id_fd(dev_cache_id, false);
		...
		int try_add, int update_path)
{
	...
	if (is_block_device(core_device)) {
	...
	if (is_block_device(core_device)) {
		...
		if (strncpy_s(cmd.core_path_name, sizeof(cmd.core_path_name), core_device, MAX_STR_LEN) != 0) {
			return FAILURE;
		}
	}
	...
	if (is_block_device(core_device)) {//TODO:
	...
	if (run_ioctl(KCAS_IOCTL_INSERT_CORE, &cmd, sizeof(cmd)) < 0) {
		...
		bool detach, bool force_no_flush)
{
	...
	if (run_ioctl_interruptible(KCAS_IOCTL_REMOVE_CORE, &cmd, sizeof(cmd),
...
void check_cache_state_incomplete(uint32_t cache_id)
{
		...
		bool force)
{
	...
	if (run_ioctl(KCAS_IOCTL_REMOVE_INACTIVE, &cmd, sizeof(cmd)) < 0) {
...
int core_pool_remove(const char *core_device)
{
	...
	if (run_ioctl(KCAS_IOCTL_CORE_POOL_REMOVE, &cmd, sizeof(cmd)) < 0) {
...
int purge_cache(uint32_t cache_id)
{
	...
	cmd.cache_id = cache_id;
	/* synchronous flag */
	if (run_ioctl_interruptible(KCAS_IOCTL_PURGE_CACHE, &cmd, sizeof(cmd), "Purging cache",
			cache_id, OCF_CORE_ID_INVALID) < 0) {
		print_err(cmd.ext_err_code);
		return FAILURE;
	}
...
int flush_cache(uint32_t cache_id)
{
	...
	cmd.cache_id = cache_id;
	/* synchronous flag */
	if (run_ioctl_interruptible_retry(KCAS_IOCTL_FLUSH_CACHE, &cmd, sizeof(cmd), "Flushing cache",
			cache_id, OCF_CORE_ID_INVALID) < 0) {
		if (OCF_ERR_FLUSHING_INTERRUPTED == cmd.ext_err_code) {
...
int purge_core(uint32_t cache_id, unsigned int core_id)
{
	...
	/* synchronous flag */
	if (run_ioctl_interruptible(KCAS_IOCTL_PURGE_CORE, &cmd, sizeof(cmd), "Purging core", cache_id, core_id) < 0) {
...
int flush_core(uint32_t cache_id, unsigned int core_id)
{
	...
	/* synchronous flag */
	if (run_ioctl_interruptible_retry(KCAS_IOCTL_FLUSH_CORE, &cmd, sizeof(cmd), "Flushing core", cache_id, core_id) < 0) {
		if (OCF_ERR_FLUSHING_INTERRUPTED == cmd.ext_err_code) {
			cas_printf(LOG_ERR, DIRTY_FLUSHING_WARNING);
			return INTERRUPTED;
		} else {
...
int partition_list(uint32_t cache_id, unsigned int output_format)
{
	...
	for (i = 0; i < OCF_USER_IO_CLASS_MAX; i++, io_class.ext_err_code = 0) {
		...
		result = run_ioctl(KCAS_IOCTL_PARTITION_INFO, &io_class, sizeof(io_class));
		if (result) {
...
int partition_set_config(struct kcas_io_classes *cnfg)
{
	...
	result = run_ioctl(KCAS_IOCTL_PARTITION_SET, cnfg, sizeof(*cnfg));
	if (result) {
...
int reset_counters(uint32_t cache_id, unsigned int core_id)
{
	...
	if (run_ioctl(KCAS_IOCTL_RESET_STATS, &cmd, sizeof(cmd)) < 0) {
...
int get_core_pool_count(void)
{
	struct kcas_core_pool_count cmd;

	if (run_ioctl(KCAS_IOCTL_GET_CORE_POOL_COUNT, &cmd, sizeof(cmd)) < 0)
		return 0;
...
int get_core_pool_devices(struct kcas_core_pool_path *cmd)
{
	...
	cmd->core_pool_count = get_core_pool_count();
	...
	if ((status = run_ioctl(KCAS_IOCTL_GET_CORE_POOL_PATHS, cmd, sizeof(*cmd))) < 0) {
...
int standby_detach(uint32_t cache_id)
{
	...
	if (run_ioctl(KCAS_IOCTL_STANDBY_DETACH, &cmd, sizeof(cmd)) < 0) {
...
int standby_activate(uint32_t cache_id, const char *cache_device)
{
	...
	if (run_ioctl(KCAS_IOCTL_STANDBY_ACTIVATE, &cmd, sizeof(cmd)) < 0) {
