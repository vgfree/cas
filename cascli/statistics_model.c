...
static void print_core_conf(const struct kcas_core_info *info, FILE *outfile)
{
	...
	print_kv_pair(outfile, "Exported Object", "%s",
				info->exported_object_exists ? exp_obj : "-");
			  ...
			  unsigned int stats_filters)
{
	...
	if (io_class_id != OCF_IO_CLASS_INVALID) {
		...
		ret = run_ioctl(KCAS_IOCTL_PARTITION_INFO, &info, sizeof(info));
		...
		if (run_ioctl(KCAS_IOCTL_GET_STATS, &stats, sizeof(stats)) < 0)
			return FAILURE;
	...
	for (part_iter_id = 0; part_iter_id < OCF_USER_IO_CLASS_MAX; part_iter_id++) {
		...
		ret = run_ioctl(KCAS_IOCTL_GET_STATS, &stats, sizeof(stats));
		if (ret)
			return FAILURE;
		      ...
		      bool by_id_path)
{
	...
	if (!standby) {
		if (run_ioctl(KCAS_IOCTL_GET_STATS, &cache_stats, sizeof(cache_stats))) {
	...
	if (stats_filters & STATS_FILTER_CONF)
		cache_stats_conf(cache_info, cache_id, outfile, by_id_path);
	...
	if ((cache_info->info.state & (1 << ocf_cache_state_incomplete))
		...
		cache_stats_inactive_usage(cache_info, cache_id, outfile);
	}
		      ...
		      FILE *outfile, unsigned int stats_filters, bool by_id_path)
{
	...
	if (get_core_info(cache_id, core_id, &core_info, by_id_path)) {
 ...
 *
 * this routine implements -P (--stats) subcommand of cascli.
		 ...
		 unsigned int stats_filters, unsigned int output_format, bool by_id_path)
{
	...
	if (stats_filters & STATS_FILTER_IOCLASS) {
		if (cache_stats_ioclasses(&cache_info, cache_id,
					core_id, io_class_id,
					intermediate_file[1],
					stats_filters)) {
			ret = FAILURE;
		...
		if (cache_stats(&cache_info, cache_id, intermediate_file[1],
					stats_filters, by_id_path)) {
			ret = FAILURE;
			goto cleanup;
		}
	} else {
		if (cache_stats_cores(&cache_info, cache_id, core_id,
