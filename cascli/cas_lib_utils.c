...
void sig_handler_interrupt_flushing(int x)
{
	...
	int res = run_ioctl(KCAS_IOCTL_INTERRUPT_FLUSHING, &cmd_info, sizeof(cmd_info));
		...
		char *friendly_name, uint32_t cache_id, int core_id, bool retry)
{
	...
	ioctl_res = run_ioctl(command, cmd, len);
 ...
 */
int run_ioctl(int command, void *cmd, size_t len)
{
	...
	length = h.length - sizeof(h);

	if (length <= 0 || length > len)
		goto out_close;

	rv = do_read(fd, cmd, length);
	if (rv < 0)
		goto out_close;
